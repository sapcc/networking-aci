...
class AllocationsManager(object):
    ...
    def _sync_allocations(self):
        ...
        with db_api.CONTEXT_WRITER.using(ctx) as session:
            ...
            no_net_allocations = {}  # same as allocations for allocs that don't have a network assigned
            ...
            for hg_name, hg_config in ACI_CONFIG.hostgroups.items():
                ...
                if hg_name in allocations:
                    ...
                    if extra_ids:
                        ...
                        del_q = del_q.filter(AllocationsModel.segmentation_id.in_(extra_ids))
                        del_q.delete()
